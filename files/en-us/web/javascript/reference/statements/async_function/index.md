...
Async functions run concurrently, and internal timers run concurrently, which means the code finishes in 2 rather than 3 seconds,
...
Please note that asynchronous APIs and async functions are different. It's important to avoid confusion.
Async functions do not run in parallel but rather run concurrently using the microtask mechanism in the event loop.
On the other hand, asynchronous APIs such as `setTimeout` and `fetch` can run in parallel. These functions are provided by runtime environments like browsers and Node.js, meaning they are not native ECMAScript features.
By itself, JavaScript code cannot run in parallel without the use of the Worker API. However, in the browser, Web APIs, and in Node.js, runtime APIs (implemented in C++ or Rust) can run in parallel.
